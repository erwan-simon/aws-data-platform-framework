...
class NativePythonProcessingWrapper(BaseProcessingWrapper):
            ...
            if file_type == "csv":
                if input_file_path.startswith("s3://"):
                    return wr.s3.read_csv(
                        input_file_path, delimiter=csv_delimiter)
                return pd.read_csv(
                    input_file_path, delimiter=csv_delimiter)
    ...
    def ingest(self, full_table_name: str, dataframe: pd.DataFrame,
               recursive_counter: int = 1,
               override_ingestion_mode: str = None,
               force_maintenance: bool = False):
        final_ingestion_mode = override_ingestion_mode \
            if override_ingestion_mode else self.output_tables[
        ...
        try:
            partitioned_dfs = self._create_partitioned_dataframes(
                dataframe.copy(), self.output_tables[full_table_name].get(
                    "partition_keys", []))
            for index, df_chunk in enumerate(partitioned_dfs):
                wr.athena.to_iceberg(
                    ...
                    partition_cols=self.output_tables[full_table_name].get(
                        "partition_keys", []),
                    merge_cols=[] if self.output_tables[full_table_name][
            ...
            self.logger.warning(
                "Athena encountered a concurrency issue with Iceberg, "
                "retrying...")
            ...
            if recursive_counter > self.MAX_INGESTION_RETRIES:
                raise ValueError(
                    ...
                    "ingestion: " + str(error)) from error
        ...
        wr.catalog.upsert_table_parameters(
            ...
            boto3_session=self.boto_session, parameters={
...
def job_entrypoint_function(execute=True) -> (
        NativePythonProcessingWrapper, object):
