...
class SparkProcessingWrapper(BaseProcessingWrapper):
    ...
    def __post_init__(self):
        ...
        self.boto_session = boto3.session.Session(region_name=self.aws_region_name)
    ...
    def ingest(
        ...
        if override_ingestion_mode:
            ...
            ingestion_mode = self.output_tables[full_table_name]["ingestion_mode"]
        ...
        if self.output_tables[full_table_name]["partition_keys"]:
            partition_keys_query_string = ", ".join(
                self.output_tables[full_table_name]["partition_keys"]
            )
        else:
    ...
    def create_table_and_ingest_data(
        self,
        long_database_name: str,
        short_database_name: str,
        table_name: str,
        partition_keys_query_string: str,
    ):
        ...
        partitioned_by_partition_keys_query_string = (
            ""
            if not partition_keys_query_string
            else f"PARTITIONED BY ({partition_keys_query_string})"
        )
        cluster_by_partition_keys_query_string = (
            ""
            if not partition_keys_query_string
            else f" CLUSTER BY ({partition_keys_query_string})"
        )
        sql_query = f"""
    ...
    def ingest_data(
        ...
        if ingestion_mode == "append":
            ...
            sql_query = (
                "INSERT OVERWRITE " + f"glue_catalog.{long_database_name}.{table_name} "
            ...
            for upsert_key in upsert_keys:
                if (
                    upsert_key
                    not in self.spark_session.sql(
                        "SELECT * FROM dataset_to_ingest"
                    ).columns
                ):
                    raise ValueError(
                        f"'{upsert_key}' upsert key "
            ...
            raise ValueError(f"Unknown ingestion mode named '{ingestion_mode}'.")
        self.logger.info("Executing sql request" + sql_query)
        self.spark_session.sql(sql_query)
...
def sql_entrypoint_processing_function(job: SparkProcessingWrapper):
    ...
    with open("task_code/main.sql") as sql_request_file:
        sql_request_string = sql_request_file.read().format(
            **job.build_sql_template_context(f"glue_catalog.{job.long_database_prefix}")
        )
    return {
...
def job_entrypoint_function(execute=True) -> (SparkProcessingWrapper, object):
    try:
