...
def update_foreign_linked_databases(logger: logging.Logger, boto_session: boto3.session.Session):
    """
    ...
    all_databases_list = []
    for databases_dict in glue_client.get_paginator(
            "get_databases").paginate(ResourceShareType='ALL'):
    ...
    for database_dict in all_databases_list:
        if database_dict["CatalogId"] == current_account_id and \
                "TargetDatabase" in database_dict.keys():
            orphan_linked_databases[database_dict["Name"]] = database_dict
        elif database_dict["CatalogId"] != current_account_id:
            foreign_databases_missing_linked_database[database_dict["Name"]] = database_dict
    for database_dict in all_databases_list:
        database_name = database_dict["Name"]
        if database_name in orphan_linked_databases.keys() and \
                database_name in foreign_databases_missing_linked_database:
    ...
    for database_name, database_dict in orphan_linked_databases.items():
        ...
        logger.info(f"Deleted orphan database '{database_name}' " +
                    f"from foreign account {database_dict['TargetDatabase']['CatalogId']}")
    for database_name, database_dict in foreign_databases_missing_linked_database.items():
        ...
        logger.info(f"Created linked database '{database_name}' " +
...
def command_line_update_foreign_linked_databases(ctx):
    update_foreign_linked_databases(
        ctx.obj.logger, ctx.obj.boto_session)
