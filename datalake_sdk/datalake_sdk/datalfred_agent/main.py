...
def instanciate_agent(logger,
    ...
    agent = Agent(
        ...
        tools=[data_analyst_agent, run_guy_agent, code_debugger_agent, file_read, file_write],
...
@click.option(
    "-s", '--model-size', required=False, default="large",
...
@click.pass_context
def command_line_datalfred_agent(ctx, model_size="large", print_sub_agent_debug=False, session_id: str = None, user_prompt: str = None):
    response = main(
        ctx.obj.logger,
