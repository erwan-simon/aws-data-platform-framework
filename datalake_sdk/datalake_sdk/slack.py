...
def send_slack_message(
    ...
    try:
        secret_string = secretsmanager_client.get_secret_value(SecretId=secret_id)[
            "SecretString"
        ]
    except ClientError as error:
        if error.response["Error"]["Code"] == "ResourceNotFoundException":
            logging.error(
                f"Slack alerting secret '{secret_id}' not found — skipping Slack notification."
            )
            return
        raise
    slack_info = json.loads(secret_string)
    channel_id = channel_id if channel_id else slack_info.get("slack_channel_id")
    if not channel_id:
        raise ValueError(
            f"Did not find any slack channel in secret {secret_id} nor in sent parameters."
        )
...
@click.option("-m", "--message-content", required=True, help="Message to send to slack")
@click.pass_context
def command_line_send_slack_message(
    context: click.Context, message_content: str, channel_id: str
) -> None:
