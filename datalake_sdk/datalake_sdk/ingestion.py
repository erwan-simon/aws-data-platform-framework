...
@click.option("-t", "--table-name", required=True, help="Name of the table")
@click.option(
    "-f",
    "--input-file-path",
    required=True,
...
def command_line_ingest(
    ...
    if use_spark:
        from datalake_sdk.spark_processing_wrapper import (
            SparkProcessingWrapper as wrapper_class,
        )
    else:
        from datalake_sdk.native_python_processing_wrapper import (
            NativePythonProcessingWrapper as wrapper_class,
        )
