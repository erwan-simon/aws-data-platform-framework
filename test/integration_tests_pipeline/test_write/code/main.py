...
try:
    ...
    raise Exception("Failed to import pipeline_utils.main. Check that you are installing the pipeline_utils library from private repository and not the public one")
...
def convert_list_to_dataframe(
        ...
        data: list) -> Union[pd.DataFrame, "DataFrame"]:
    if IS_SPARK_ENVIRONMENT:
        return job.spark_session.createDataFrame(
            data=data, schema=StructType([
...
def test_upsert(job: BaseProcessingWrapper,
                database_name: str, table_prefix: str):
    ...
    assert df[
        df["name"] == "Michel"].iloc[0]["city"] == "Lyon"
    assert df[
