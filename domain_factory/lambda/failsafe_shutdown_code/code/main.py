...
def handle_ecs(logger: logging.Logger,
               environment_name: str,
               event: dict) -> (str, str):
    step_function_task_token = [
        environment_variable_dict["value"]
        for environment_variable_dict in
        event["detail"]["overrides"][
            "containerOverrides"][0]["environment"]
        if environment_variable_dict["name"] ==
        "step_function_task_token"][0]
    try:
        error_message = event['detail']['lastStatus']
        task_name = event["detail"]["clusterArn"].split(":")[-1].replace(
            f"cluster/{environment_name}_", "")
        ...
        error_message = f"ECS task {task_name} ended in STOPPED state with" + \
            f" status code '{exit_status_code}': {task_error_message}"
    except KeyError as error:
        error_message = f"Platform error: Did not find key '{error}' " + \
            "in ECS stopping event"
...
def handle_emr_serverless(_: logging.Logger,
                          environment_name: str,
                          event: dict) -> (str, str):
    ...
    job_run_config = emr_serverless_client.get_job_run(
        ...
        jobRunId=event["detail"]["jobRunId"])[
            "jobRun"]
    application_config = emr_serverless_client.get_application(applicationId=job_run_config["applicationId"])["application"]
    task_name = application_config["name"].replace(f"{environment_name}_", "")
    entrypoint_arguments = json.loads(job_run_config[
        "jobDriver"]["sparkSubmit"]["entryPointArguments"][0])
    step_function_task_token = entrypoint_arguments[
        "step_function_task_token"]
    task_error_message = job_run_config.get(
        "stateDetails", "No state details found in EMR job run")
    error = f"EMR task {task_name} ended in {job_run_config['state']} state: " + \
        task_error_message
...
TRIAGE_DICT = {
    "aws.ecs": handle_ecs,
    "aws.emr-serverless": handle_emr_serverless
}

def investigate_error_with_datalfred(logger, boto_session, project_name: str, domain_name: str, stage_name: str):
    model_size = "large"
    print_sub_agent_debug = True
    response = datalfred_main(
        logger, boto_session, project_name, domain_name, stage_name, model_size,
        print_sub_agent_debug, f"Can you tell me why the pipeline of the project {project_name}, "
...
def main(event: dict, _: dict):
    ...
    logger.info(event)
    try:
        step_function_task_token, error_message = TRIAGE_DICT[
            event["source"]](
    ...
    try:
        ...
        logger.error("The datalfred investigation did not seem to work: " + str(error))
    # error parameter string size is capped to 256 characters
    try:
        sfn_client.send_task_failure(
            taskToken=step_function_task_token,
