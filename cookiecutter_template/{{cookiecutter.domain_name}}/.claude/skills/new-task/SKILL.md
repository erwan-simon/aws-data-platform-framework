...
5. **Generate files.**
   ...
   - **`iac/<PIPELINE>/<name>/code/main.sql`** (sql) — minimal `SELECT * FROM {database_prefix}<domain>.<first_input_table>` with a comment showing the output table will receive whatever this query returns. Reject if no input tables provided (SQL tasks always need at least one). Templated placeholders available in `main.sql` (rendered via Python `str.format()` before submission): `{database_prefix}` — the Glue database prefix, prepend it to every table reference; `{logical_date}` — the Step Functions execution start date as `YYYY-MM-DD`, use it for idempotent date filtering, e.g. `WHERE event_date = DATE '{logical_date}'`.
